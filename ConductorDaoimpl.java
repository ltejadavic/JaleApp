...
public class ConductorDaoimpl implements IConductorDao {
	...
	public List<Conductor> findByNameConductor(Conductor co) {
		...
		try {
			...
			Query q = em.createQuery("from Conductor m where m.nameConductor like ?1");
			q.setParameter(1, "%" + co.getNameConductor() + "%");
			lista = (List<Conductor>) q.getResultList();

		} catch (Exception e) {
			System.out.println("Error al buscar Conductores");
